  ...
  const NavItem = ({ item }) => {
    ...
    return (
      <NavLink
        ...
        className={({ isActive }) =>
          cn(
            'flex items-center gap-2 px-2 py-1.5 sm:px-3 sm:py-2 rounded text-xs sm:text-sm font-medium transition-colors',
            isActive
  ...
  return (
    ...
    >
      ...
      <div className="sidebar-header">
        ...
        {collapsed && (
          <div className="w-6 h-6 sm:w-8 sm:h-8 bg-primary-500 rounded flex items-center justify-center">
            <Database className="w-3.5 h-3.5 sm:w-5 sm:h-5 text-white" />
          </div>
        )}
      </div>
      ...
      <div className="sidebar-content">
        {navItems.map((section) => (
          <div key={section.category} className="mb-4 sm:mb-6">
            {!collapsed && (
              <h3 className="px-2 sm:px-3 mb-1.5 sm:mb-2 text-xs font-semibold text-gray-500 uppercase tracking-wider">
                {section.category}
              </h3>
            )}
      ...
      <div className="sidebar-footer">
        {!collapsed && (
          <div className="flex items-center gap-2 px-2 py-1.5 sm:px-3 sm:py-2">
            <div className="w-6 h-6 sm:w-8 sm:h-8 bg-gradient-to-br from-primary-500 to-secondary-500 rounded-full flex items-center justify-center">
              <span className="text-white text-xs sm:text-sm font-medium">R</span>
            </div>
